...
Инструкция по развёртыванию Helm-чарта для проекта **account-number-app** с использованием Minikube.
...
```
```bash
    docker build -t account-number-app:v1 .
```
...
```bash # Установка чарта в namespace warehouse-ns
   helm install account-number ./deployment/helm/ --namespace account-number-ns --create-namespace
```

Если чарт уже установлен:

```bash
   helm upgrade account-number ./deployment/helm/ --namespace account-number-ns
```
...
```bash
   kubectl get all -n account-number-ns
```

### 🔧 Проверка PostgreSQL StatefulSet

```bash
   kubectl get statefulsets -n account-number-ns
   kubectl get pvc -n account-number-ns
```
...
```bash
   helm uninstall account-number -n account-number-ns
   kubectl delete namespace account-number-ns
```
...
```bash
   kubectl describe pod $(kubectl get pods -n account-number-ns --no-headers | sed -n '1p' | awk '{print $1}') -n account-number-ns
```

```bash
   kubectl logs $(kubectl get pods -n account-number-ns --no-headers | sed -n '1p' | awk '{print $1}') -n account-number-ns
```


```bash
   kubectl port-forward -n account-number-ns pod/$(kubectl get pods -n account-number-ns --no-headers | sed -n '1p' | awk '{print $1}') 8084:8084
```