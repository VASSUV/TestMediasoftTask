...
Инструкция по развёртыванию Helm-чарта для проекта **delivery-app** с использованием Minikube.
...
```
```bash
    docker build -t delivery-app:v2 .
```
...
```bash # Установка чарта в namespace warehouse-ns
   helm install delivery ./deployment/helm/ --namespace delivery-ns --create-namespace
```

Если чарт уже установлен:

```bash
   helm upgrade delivery ./deployment/helm/ --namespace delivery-ns
```
...
```bash
   kubectl get all -n delivery-ns
```

### 🔧 Проверка PostgreSQL StatefulSet

```bash
   kubectl get statefulsets -n delivery-ns
   kubectl get pvc -n delivery-ns
```
...
```bash
   helm uninstall delivery -n delivery-ns
   kubectl delete namespace delivery-ns
```
...
```bash
   kubectl describe pod $(kubectl get pods -n delivery-ns --no-headers | sed -n '1p' | awk '{print $1}') -n delivery-ns
```

```bash
   kubectl logs $(kubectl get pods -n delivery-ns --no-headers | sed -n '1p' | awk '{print $1}') -n delivery-ns
```


```bash
   kubectl port-forward -n delivery-ns pod/$(kubectl get pods -n delivery-ns --no-headers | sed -n '1p' | awk '{print $1}') 8086:8086
```
```bash
   kubectl port-forward -n delivery-ns service/delivery-delivery-service 8086:8086
```
...
curl --header "Content-Type: application/json" --request POST --data  '{"inn":"value1", "accountNumber":"value2"}' http://delivery-delivery-service.delivery-ns.svc.cluster.local:8082/api/register
curl --header "Content-Type: application/json" --request POST --data  '{"inn":"value1", "accountNumber":"value2"}' http://delivery-delivery-service.delivery-ns.svc.cluster.local:8085/api/register
curl --header "Content-Type: application/json" --request POST --data  '{"inn":"value1", "accountNumber":"value2"}' http://delivery-delivery-service.delivery-ns.svc.cluster.local:8085
